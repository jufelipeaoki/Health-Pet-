...
$nome2 = $raca2 = $sexo2 = $data2 = $especie2  = $id2 = "";
if ($_SERVER["REQUEST_METHOD"] == "POST" || isset($_SESSION['usuario'])) {
    ...
    try {
        ...
        $con = $conexao->prepare("SELECT * FROM pet WHERE cliente_pet = ? AND numero_pet=2");
...
function editarUsuario($campo, $valor) {
    try {
        ...
        $sql = "UPDATE pet SET $campo = :valor WHERE cliente_pet = $_SESSION[usuario] AND numero_pet=2";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['edit_nome'])) {
        $nome2 = $_POST['nome'];
        if (editarUsuario('pet_nome', $nome2)) {
        ...
        $raca2 = $_POST['raca'];
        if (editarUsuario('pet_raca', $raca2)) {
    ...
    elseif(isset($_POST['edit_data'])) {
        $data2 = $_POST['data'];
        if (editarUsuario('data_pet', $data2)) {
...
<header class="header">
  ...
  <a href="logout.php"><span class="home">|Sair</span></a>
    ...
    <div class="pet1">
        <form method="POST">
       <button class="consulte" style="height:50px; width:70px" name="consulte"><?php echo($nome2) ?></button>
        <ul>
                <span class="one">ID/Código do Pet: </span>
                <input type="text" name="id" placeholder="<?php echo($id2);  ?>"> <span>Essa informação será utilizada para realizar agendamentos para seu Pet</span><br><br>
                <span class="one">Nome do Pet: </span>
                <input type="text" name="nome" placeholder="<?php echo($nome2);  ?> "> <button type="submit" name="edit_nome" class="consulte"> <i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Raça:</span>
                <input type="text" name="raca" placeholder=" <?php echo($raca2);  ?>"> <button type="submit" name="edit_raca" class="consulte"><i class="fa-solid fa-pencil"></i></button><br><br>
                <span class="one"> Meu pet é...</span>
                <input type="text" name="sexo" placeholder=" <?php echo($sexo2);  ?>"> <button type="submit" name="edit_sexo" class="consulte"><i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Data de Nascimento:</span>
                <input type="text" name="data" placeholder="<?php echo($data2);  ?>" > <button type="submit" name="edit_data" class="consulte"><i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Espécie: </span>
                <input type="data" name="especie" placeholder="<?php echo($especie2);?>">
