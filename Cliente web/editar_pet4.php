...
$nome4 = $raca4 = $sexo4 = $data4 = $especie4  = $id4 = "";
if ($_SERVER["REQUEST_METHOD"] == "POST" || isset($_SESSION['usuario'])) {
    ...
    try {
        ...
        $con = $conexao->prepare("SELECT * FROM pet WHERE cliente_pet = ? AND numero_pet=4");
...
function editarUsuario($campo, $valor) {
    try {
        ...
        $sql = "UPDATE pet SET $campo = :valor WHERE cliente_pet = $_SESSION[usuario] AND numero_pet=4";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['edit_nome'])) {
        $nome4 = $_POST['nome'];
        if (editarUsuario('pet_nome', $nome4)) {
        ...
        $raca4 = $_POST['raca'];
        if (editarUsuario('pet_raca', $raca4)) {
    ...
    elseif(isset($_POST['edit_data'])) {
        $data4 = $_POST['data'];
        if (editarUsuario('data_pet', $data4)) {
...
<header class="header">
  ...
  <a href="logout.php"><span class="home">|Sair</span></a>
    ...
    <div class="pet1">
        <form method="POST">
       <button class="consulte" style="height:50px; width:70px" name="consulte"><?php echo($nome4) ?></button>
        <ul>
                <span class="one">ID/Código do Pet: </span>
                <input type="text" name="id" placeholder="<?php echo($id4);  ?>"> <span>Essa informação será utilizada para realizar agendamentos para seu Pet</span><br><br>
                <span class="one">Nome do Pet: </span>
                <input type="text" name="nome" placeholder="<?php echo($nome4);  ?> "> <button type="submit" name="edit_nome" class="consulte"> <i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Raça:</span>
                <input type="text" name="raca" placeholder=" <?php echo($raca4);  ?>"> <button type="submit" name="edit_raca" class="consulte"><i class="fa-solid fa-pencil"></i></button><br><br>
                <span class="one"> Meu pet é...</span>
                <input type="text" name="sexo" placeholder=" <?php echo($sexo4);  ?>"> <button type="submit" name="edit_sexo" class="consulte"><i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Data de Nascimento:</span>
                <input type="text" name="data" placeholder="<?php echo($data4);  ?>" > <button type="submit" name="edit_data" class="consulte"><i class="fa-solid fa-pencil"></i> </button><br><br>
                <span class="one">Espécie: </span>
                <input type="data" name="especie" placeholder="<?php echo($especie4);?>">
